...
function buildPrompt(action: string, p: { text: string; question: string; title: string }) {
  ...
  switch (action) {
    ...
    case "flashcards":
      return `${base}\n\nأنشئ من الكتاب «${p.title}» بطاقات مراجعة تعليمية (١٠ إلى ١٥ بطاقة) تغطّي أهم أفكاره ومصطلحاته. ` +
        `أعِد النتيجة بصيغة JSON فقط، دون أي نص قبله أو بعده، على هيئة مصفوفة عناصرها {"q":"سؤال أو مصطلح موجز","a":"الجواب أو التعريف المختصر"}.\n\n=== نص الكتاب ===\n${book}`;
    case "quiz":
      return `${base}\n\nأنشئ اختباراً من متعدّد الخيارات (٦ إلى ٨ أسئلة) على محتوى الكتاب «${p.title}». ` +
        `أعِد النتيجة بصيغة JSON فقط دون أي نص إضافي، مصفوفة عناصرها {"q":"نص السؤال","options":["أ","ب","ج","د"],"correct":0,"why":"سبب موجز للإجابة الصحيحة"} حيث correct فهرس الخيار الصحيح (يبدأ من 0).\n\n=== نص الكتاب ===\n${book}`;
    case "translate":
      return `${base}\n\nترجم النص التالي إلى ${p.question || "الإنجليزية"} ترجمةً دقيقة وطبيعية، وأعد الترجمة فقط دون شرح:\n\n«${p.text.slice(0, 4000)}»`;
    case "library":
      return `${base}\n\nأجب عن سؤال القارئ اعتماداً على المقتطفات التالية المأخوذة من كتب مكتبته (كل مقتطف مسبوق بمصدره). ` +
        `اذكر في جوابك من أي كتاب استقيت المعلومة. إن لم تكفِ المقتطفات فاذكر ذلك.\n\nالسؤال: ${p.question}\n\n=== مقتطفات من المكتبة ===\n${book}`;
    default:
      return `${base}\n\n${p.text}`;
  }
}
